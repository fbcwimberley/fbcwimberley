<script lang="ts">
	...
	onMount(() => {
		...
		const stopWatchingSchedule = watchGatheringSchedule(() => {
			gatheringSchedule = getGatheringSchedule();
		});

		return () => {
			window.clearTimeout(preloadTimer);
			stopWatchingSchedule();
		};
	});
</script>
...
<section class="relative min-h-screen flex items-end justify-center overflow-hidden">
	...
	<div class="container relative z-2 text-center pb-24">
		...
		<p class="text-[clamp(1rem,2vw,1.35rem)] text-white/90 mb-3 font-normal tracking-[0.05em]">{gatheringSchedule.hero}</p>
